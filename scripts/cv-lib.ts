// Shared CV rendering: HTML template, CSS and PDF generation.
// Consumed by both build-cv.ts (Russian, loaded from docs/) and
// build-cv-en.ts (English, inline snapshot). Keeping a single template here
// guarantees both language variants share the exact same layout.
...
/** Language-specific UI strings (section titles, badges, header chips). */
export interface Labels {
  ...
  nowBadge: string;
  /** Header chips; receives years of experience for the first chip. */
  chips: (yearsInIT: number) => string[];
}
...
function renderExperience(experiences: Experience[], nowBadge: string): string {
  return experiences
    .map((e) => {
      ...
      <div class="exp${isCurrent ? " exp-current" : ""}">
        <div class="exp-row">
          ...
          <div class="exp-right">
            ${isCurrent ? `<span class="exp-now">${esc(nowBadge)}</span>` : ""}
...
function renderSkills(
  ...
  return skills
    .map((s) => {
      const name = skillNames[s.category] ?? s.category;
      const tags = skillTags[s.category] ?? extractBoldTerms(s.content).slice(0, 7);
      return `
        <div class="skill-cat">
...
export function buildHtml(
  ...
  const { profile, summary, strengths, lookingFor, education, experiences, skills } = data;
  ...
  const chips = labels
    .chips(yearsInIT)
    .map((c) => `<span class="hd-chip">${esc(c)}</span>`)
    .join("");

  // Render "looking for" from data — single sentence per paragraph
...
<body>
  ...
  <div class="body">
    ...
    <div class="main">
      ...
      <div class="section">
        <div class="sec-title">${esc(labels.sections.about)}</div>
        <div class="sum-text">${summaryParas}</div>
        ${strengthsHtml}
      </div>

      <div class="section">
        <div class="sec-title">${esc(labels.sections.experience)}</div>
        ${renderExperience(experiences, labels.nowBadge)}
      </div>
    ...
    <div class="sidebar">
      ...
      <div class="section">
        <div class="sec-title">${esc(labels.sections.skills)}</div>
        ${renderSkills(skills, skillNames, skillTags)}
      </div>

      <div class="section">
        <div class="sec-title">${esc(labels.sections.education)}</div>
        ${renderEducation(education)}
      </div>

      <div class="section">
        <div class="sec-title">${esc(labels.sections.languages)}</div>
        ${renderLanguages(education)}
      </div>

      <div class="section">
        <div class="sec-title">${esc(labels.sections.looking)}</div>
...
export async function generatePdf(html: string, outPath: string): Promise<void> {
